...
def kdtree(outlier_cloud):
    ...
    for i in range(len(outlier_cloud.points)):
        ...
        [k, indices, _] = pcd_tree.search_knn_vector_3d(query_point, 300)  # Perform KNN search
    ...
    for cluster_label in unique_labels:
        cluster_indices = np.where(labels == cluster_label)[0]  # Get the indices of points in the cluster
    ...
    outlier_cloud.colors = o3d.utility.Vector3dVector(colors)#cluster_colors[:, :3])
...
clustered_cloud, labels = dbscan(outlier_cloud)
o3d.visualization.draw_geometries([inlier_cloud, clustered_cloud])
list_of_visuals = bounding_boxes(clustered_cloud,labels)
